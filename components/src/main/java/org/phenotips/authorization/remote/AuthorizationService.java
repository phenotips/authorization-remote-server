...
public interface AuthorizationService
{
    ...
    /**
     * Checks if the current user has the requested access level on the target patient.
     * 
     ...
     */
    Boolean hasAccess(String access, Patient patient);

    /**
     * Checks if the specified user has the requested access level on the target patient.
     * 
     * @param access the requested access level
     * @param username the user whose rights should be checked, specified as a username; serialized references are
     *            accepted as well
     * @param patient the target patient
     * @return {@code True} if access is granted, {@code False} if access is denied, {@code null} if this component
     *         cannot determine if access should be granted or denied
     */
    Boolean hasAccess(String access, String username, Patient patient);
     ...
     *         cannot determine if access should be granted or denied
     */
    Boolean hasAccess(String access, User user, Patient patient);
}
